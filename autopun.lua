    ...
    downloadUrlToFile(update_url, update_path, function (id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateini.info.vers) > script_vers then 
                message("Íàéäåíî îáíîâëåíèå. Âåðñèÿ:{FFA500} "..updateini.info.vers_text)
                update_state = true
            end
    ...
    while true do
        ...
        if update_state then
			downloadUrlToFile(script_url, script_path, function (id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					message("Îáíîâëåíèå óñïåøíî óñòàíîâëåíî.")
					update_state = false
					thisScript():reload()
				end
			end)
    ...
    if command_from_admin_chat_status == true then
            ...
            if adm_chat_cmd_player_id ~= nil then
                if adm_chat_cmd_player_val ~= nil then
                    ...
                    lua_thread.create(function()
                        ...
                        text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò èçìåíèòü HP èãðîêó {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                        message(text)
                        message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                        lasttime = os.time()
                        lasttimes = 0
                        time_out = 15
                        while lasttimes < time_out do
                            ...
                            active_forma = true
                            if stop_forma then
                                message("Êîìàíäó âûïîëíèë äðóãîé àäìèíèñòðàòîð.")
                                stop_forma = false
                                break
                            end
                            if lasttimes == time_out then
                                message("Âðåìÿ îæèäàíèÿ èñòåêëî.")
                            end
                            ...
                            if isKeyJustPressed(VK_K) and not sampIsChatInputActive() and not sampIsDialogActive() then
                                ...
                                wait(200)
                                sampSendChat(string.format("/a %s, êîìàíäà ïðèìåíåíà.", adm_name))
                                --sampSendChat("/ans "..adm_chat_cmd_player_id.. " HP âûäàíî ïî ïðîñüáå àäìèíèñòðàòîðà "..adm_name)
                                active_forma = false
                                break
                            end
                        end
                ...
                if term ~= nil then
                    if reason_mute == nil then
                        ...
                        lua_thread.create(function()
                            wait(100)
                            adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                            text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò ïîñòàâèòü çàòû÷êó èãðîêó {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                            message(text)
                            message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                            lasttime = os.time()
                            lasttimes = 0
                            ...
                            while lasttimes < time_out do
                                ...
                                if isKeyJustPressed(VK_K) and not sampIsChatInputActive() and not sampIsDialogActive() then
                                    ...
                                    wait(200)
                                    sampSendChat(string.format("/a %s, êîìàíäà ïðèìåíåíà.", adm_name))
                                    --sampSendChat("/ans "..adm_chat_cmd_player_id.." Íàêàçàíèå âûäàíî ïî ïðîñüáå àäìèíèñòðàòîðà "..adm_name..".")
                                    active_forma = false
                                    break
                                end
                            end
                ...
                if term ~= nil then
                    if reason_ban == nil then
                        ...
                        lua_thread.create(function()
                            wait(100)
                            adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                            text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò çàáàíèòü èãðîêà {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                            message(text)
                            message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                            lasttime = os.time()
                            lasttimes = 0
                ...
                if term ~= nil then
                    if reason_jail == nil then
                        ...
                        lua_thread.create(function()
                            wait(100)
                            adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                            text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò ïîñàäèòü â òþðüìó èãðîêà {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                            message(text)
                            message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                            lasttime = os.time()
                            lasttimes = 0
            ...
            if adm_chat_cmd_player_id ~= nil then
                    if reason_warn == nil then
                        ...
                        lua_thread.create(function()
                            wait(100)
                            adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                            text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò âûäàòü ïðåäóïðåæäåíèå èãðîêó {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                            message(text)
                            message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                            lasttime = os.time()
                            lasttimes = 0
            ...
            if adm_chat_cmd_player_id ~= nil then
                if reason_kick == nil then
                    ...
                    lua_thread.create(function()
                        wait(100)
                        adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                        text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò êèêíóòü èãðîêà {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                        message(text)
                        message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                        lasttime = os.time()
                        lasttimes = 0
        ...
        if adm_command:find("/msg") then
            ...
            if msg_text ~= nil then
                active_forma = true
                    lua_thread.create(function()
                        wait(100)
                        text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò ñäåëàòü îáúÿâëåíèå.", adm_name, adm_id)
                        message(text)
                        message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ <K>.")
                        lasttime = os.time()
                        lasttimes = 0
                        time_out = 15
                        while lasttimes < time_out do
        ...
        if adm_command:find("/skick") then
            ...
            if adm_chat_cmd_player_id ~= nil then
                ...
                lua_thread.create(function()
                    wait(100)
                    adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                    text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò êèêíóòü {FFD700}%s[%d] {FFFFFF}áåç ëèøíåãî øóìà.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                    message(text)
                    message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                    lasttime = os.time()
                    lasttimes = 0
        ...
        if adm_command:find("/unmute") then
            ...
            if adm_chat_cmd_player_id ~= nil then
                ...
                lua_thread.create(function()
                    wait(100)
                    adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                    text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò ñíÿòü çàòû÷êó ñ èãðîêà {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                    message(text)
                    message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                    lasttime = os.time()
                    lasttimes = 0
        ...
        if adm_command:find("/unban") then
            ...
            if adm_chat_cmd_player_name ~= nil then
                ...
                lua_thread.create(function()
                    wait(100)
                    text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò ðàçáàíèòü èãðîêà {ffd700}%s{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name)
                    message(text)
                    message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                    lasttime = os.time()
                    lasttimes = 0
        ...
        if adm_command:find("/unjail") then
            ...
            if adm_chat_cmd_player_id ~= nil then
                ...
                lua_thread.create(function()
                    wait(100)
                    adm_chat_cmd_player_name = sampGetPlayerNickname(tonumber(adm_chat_cmd_player_id))
                    text = string.format("Àäìèíèñòðàòîð %s[%d] õî÷åò âûïóñòèòü èç òþðüìû èãðîêà {ffd700}%s[%d]{ffffff}.", adm_name, adm_id, adm_chat_cmd_player_name, adm_chat_cmd_player_id)
                    message(text)
                    message("Íàæìèòå êëàâèøó ïîäòâåðæäåíèÿ.")
                    lasttime = os.time()
                    lasttimes = 0
